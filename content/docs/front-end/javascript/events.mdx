...
However, this approach is generally _not_ recommended because it mixes HTML and JavaScript, making your code harder to maintain.

### DOM Event Listeners (Recommended)
...
**Important:** You must pass the _same_ function reference to `removeEventListener()` that you passed to `addEventListener()` for it to work. This is why it's important to assign the event handler function to a variable.
...
When an event occurs on an element, it can propagate up the DOM tree to its parent elements. This is called **event bubbling**.

For example, if you click on a button inside a `<div>`, the click event will first be handled by the button, then by the `<div>`, and then by the `<body>`, and so on, up to the `document`.

You can stop event bubbling using the `stopPropagation()` method of the event object:

```javascript
const button5 = document.querySelector("#myButton");
...
In this example, when you click on the button, only the "Button clicked!" message will be logged. The "Div clicked!" message will not be logged because the `stopPropagation()` method prevents the event from bubbling up to the `<div>`.

**Event Capturing** is the opposite of bubbling. With capturing, the event is first captured by the outermost element and then propagates down to the target element. Capturing is less commonly used than bubbling.

## Practice and Explore

Events are the key to creating interactive web pages. Practice attaching event listeners to different elements and responding to different events. Experiment with the event object and event bubbling. The more you work with events, the better you'll understand how to create engaging and responsive user experiences.
