...
- **`<strong>` and `<em>`:** These are used to emphasize text. `<strong>` indicates strong importance (usually displayed in bold), and `<em>` indicates emphasis (usually displayed in italics). Use them semantically (based on meaning), not just for styling.

```html
<p>
  This is <strong>very important</strong> information, and you should pay
  <em>close attention</em>.
</p>
```
...
```html
<p>This is the first thematic.</p>
<hr />
<p>This is the second thematic.</p>
```
...
- **`<iframe>`**: Creates an embedded frame to display content from other webpage.
